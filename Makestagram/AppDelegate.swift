...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    override init(){
        ...
        parseLoginHelper = ParseLoginHelper {[unowned self] user, error in
            //instantiate the ParseLoginHelper with a callback
            if let error = error{
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        let configuration = ParseClientConfiguration{
            $0.applicationId = "makestagram-parse-htc"
            $0.server = "https://makestagram-parse-htc.herokuapp.com/parse"
        }
        ...
        if (user != nil){
            //if we have a user, set the TabBarController to be the initial view
            let storyboard = UIStoryboard(name: "Main", bundle: nil)
